...
# The required dependancies must be installed.

# On CentOS 7 as root run:
...
# apt-get build-dep nginx && apt-get install gnupg2
...
install () {
  cd nginx-build-files/$NGINX_VERSION
  make install
  # create cache directory so default install config works
...
if [[ $1 =~ ^(install|build)$ ]]; then
  ...
  echo "Run './nginx-pagespeed-build-script.sh build' to build"
  echo "Run './nginx-pagespeed-build-script.sh install' to install"
  exit 1
fi
